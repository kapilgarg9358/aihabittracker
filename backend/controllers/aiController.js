...
export const weeklyReport = async (req, res) => {
  try {
    ...
    .join("\n")}\n\nPlease write your personalised weekly report now.`;
...
export const chatAnalysis = async (req, res) => {
   try {
    ...
    .map((h) => {
      ...
      return `${h.name} (${h.category}): ${hLogs.length}/30 in the last 30 days, by weekday [Sun,Mon,Tue,Wed,Thu,Fri,Sat]: ${byDow.join(", ")}`;
    })
    .join("\n");
